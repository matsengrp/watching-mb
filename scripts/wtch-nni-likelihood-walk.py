...
def find_nni_trees(j, tree_bits_list): 
    ...
    """
    return [
        (j,k) for k in range(j+1, len(tree_bits_list) ) 
...
def find_likely_neighbors(sdag_rep_path, output_path, max_tree_count=0, max_tree_ratio=0.0):
    """
    ...
    with open(output_path, "wt") as out_file:
        for vertex in the_graph.vs[good_vertex_indices]:
            ...
            out_file.write(
                ",".join(map(str, sdag_rep))
                + f",{vertex['log_likelihood']}"
                + "\n"
            )
